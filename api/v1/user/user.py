...
from utils.user_info import upload_photo, obj_to_dict, user_dict
...
class UserAPI(MethodView):
    def get(self):
        ...
        return jsonify({'code': 200, 'message': 'success', 'data': user_dict(user_id)})
        ...
        if captcha_str is not None and new_pwd is not None:
            ...
            if hasattr(g, 'user') and g.user is not None:
                try:
                    ...
                    file_name = 'avatar-' + g.user.user_id + '-' + rand_str(6)
                    upload_photo(photo_obj=new_avatar, photo_name=file_name, suffix=suffix)
                    user: UserModel = UserModel.query.filter(UserModel.user_id == g.user.user_id).first()
                    user.avatar = f'https://byszqq-1310478750.cos.ap-nanjing.myqcloud.com/{file_name}{suffix}'
                    db.session.commit()
                except SQLAlchemyError as e:
                    Log.error(e)
                    db.session.rollback()
